...
function nextSundayDate() {
  const d = new Date(); const day = d.getDay(); const add = (7 - day) % 7 || 7; d.setDate(d.getDate() + add); return d;
}
...
export function TalentOverview({ onTasks }: { onTasks: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const wk = pr?.[0]; if (wk && wk.status === "pending") setWeekEarnings(Number(wk.total_amount));
    })();
  }, [user]);
    ...
    <div className="space-y-6">
      <Reveal>
        <div>
          ...
          <p className="text-sm text-muted-foreground">Your work hub — assignments, earnings and growth. Official bureau, no client hunting.</p>
        </div>
      </Reveal>

      <Stagger className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <StaggerItem><Stat icon={Briefcase} label="Active tasks" value={String(active.length)} hint={`${newOnes.length} awaiting response`} /></StaggerItem>
        <StaggerItem><Stat icon={CheckCircle2} label="Completed total" value={String(doneTotal)} /></StaggerItem>
        <StaggerItem><Stat icon={Wallet} label="This week" value={`₦${weekEarnings.toLocaleString()}`} hint="Pending payday" /></StaggerItem>
        <StaggerItem><Stat icon={CalendarClock} label="Next payday" value={`${daysToPayday}d`} hint={payday.toDateString()} /></StaggerItem>
        <StaggerItem><Stat icon={Award} label="Tier" value={TIER_NAMES[tier]?.split("·")[0] || "Tier 1"} hint={TIER_NAMES[tier]?.split("·")[1]?.trim()} /></StaggerItem>
        <StaggerItem><Stat icon={ThumbsUp} label="Approval rate" value={`${approval}%`} /></StaggerItem>
      </Stagger>
      ...
      <Reveal>
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onTasks} className="text-sm font-semibold text-primary hover:underline">See all →</button>
          </div>
          {active.length === 0 ? (
            ...
            <ul className="mt-4 divide-y divide-border">
              {active.slice(0, 5).map((t) => (
                <li key={t.id} className="flex items-center justify-between py-3">
                  <div><div className="text-sm font-semibold">{t.title}</div><div className="text-xs text-muted-foreground capitalize">{t.service_category} · {t.tier} · deadline {t.deadline || "flexible"}</div></div>
                  <div className="text-right"><div className="text-sm font-bold">₦{(t.talent_pay_rate || 0).toLocaleString()}</div><div className="text-[10px] uppercase tracking-widest text-muted-foreground">{t.status.replace("_", " ")}</div></div>
                </li>
              ))}
            </ul>
          )}
